...
class ccHandler(object):
    ...
    def openSession(self):
        '''
        Start a session with smartcard
        '''
        error = None
        for s in self.slots:
            try:
                ...
                try:
                    ...
                    if 'CKR_USER_PIN_NOT_INITIALIZED' in str(sys.exc_info()[1]) :
    ...
    def loadDict(self):
        '''
        Load all smartcard object into a dict
        '''
    ...
    def sign(self,toSign):
        '''
        Perform a digital signature with user's smartcard
        This is done using SHA1 RSA PKCS for compatibility
        with Safebox PAM
        '''
        ...
        try:
            ...
            self.signature = self.session.sign(self.key,toSign,mech)
            ...
            #print "CC MODULE: SIGNATURE %s" % sign
    ...
    def verify(self):
        '''
        Verify a given a digital signature
        '''
        ...
        for c in self.m:
            #bytechar = bytes(chr(c)).encode('hex')
            self.mx += chr(c).encode('hex')

        for c in self.e:
            self.ex += chr(c).encode('hex')
            #bytechar = bytes(chr(c)).encode('hex')
            #self.mx += bytechar
        ex = eval('0x%s' % ''.join(chr(self.e) for self.e in self.e).encode('hex'))
        #self.ex =  hex(ex)
        #print self.ex

        mx =  eval('0x%s' % ''.join(chr(c) for c in self.m).encode('hex'))
        sx = eval('0x%s' % s)
